...
import { i as flowPackageSha256, n as buildFlowReviewCard, o as validateCodexFlowPackage, r as flowMetaToRegistryShape, t as buildFlowIntakeReceipt } from "./flow-format.js";
...
function templateRoot() {
	...
	return candidates.find((c) => existsSync(join(c, "meta.yaml"))) ?? candidates.find((c) => existsSync(join(c, "contract.json"))) ?? candidates[0];
}
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (command === "add_count_rules") {
				if (existsSync(join(packageDir, "meta.yaml"))) return {
					ok: false,
					message: "add_count_rules 仅适用于旧 contract.json 格式；Codex_Flow 包不声明素材槽 count_rule"
				};
			...
			if (command === "planned_counts") {
				if (existsSync(join(packageDir, "meta.yaml"))) return {
					ok: false,
					message: "planned_counts 仅适用于旧 contract.json 格式；Codex_Flow 包以 workflow.yaml 阶段为准，不按时长推导素材计划数"
				};
			...
			if (command === "publish") {
				if (existsSync(join(packageDir, "meta.yaml"))) {
					...
					const sealed = readIntakeSources(packageDir);
					const sources = sealed ? sealed.map((s) => ({
						name: s.name,
						sha256: s.sha256
						...
						name: "curator-input",
						sha256: flowPackageSha256(packageDir)
					}];
					...
					try {
						...
						const routing = {
							...
							negative_intents: Array.isArray(shape.flow.exclude_intents) ? shape.flow.exclude_intents : []
						};
						const record = registry.ingest({
							contract,
							routing,
							packageRoot: packageDir,
							provenance: "curator (codex-flow)"
						}, { force: true });
						registry.setStatus(String(shape.name), String(shape.version), "published");
						return {
							ok: true,
							message: `published ${record.name}@${record.version} with intake receipt (codex-flow)`,
							skill: {
								id: record.id,
								name: record.name,
