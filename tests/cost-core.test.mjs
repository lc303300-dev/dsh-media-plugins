...
test('windowAt/priceAtTs apply the off-peak discount inside the Beijing window', () => {
  ...
  assert.ok(Math.abs(discounted.inputMiss - 0.5) < 1e-9)
  assert.ok(Math.abs(discounted.output - 1) < 1e-9)
...
test('stepCost prices each bucket at the effective RMB/M rate', () => {
  ...
  assert.ok(Math.abs(c.input - 2) < 1e-9)
  assert.ok(Math.abs(c.output - 8) < 1e-9)
  assert.ok(Math.abs(c.total - 10) < 1e-9)
})
