...
//#region src/shared/revision-core.ts
/**
...
function classifyFeedback(userFeedback) {
	...
	if (hasAmbiguousSignal && !hasLocalTarget) return {
		classification: "ambiguous_creative",
		reasons: ["反馈表达审美目标或不满意，但缺少可直接执行的局部修改。"]
	};
	if (hasLocalTarget) return {
		classification: "explicit_local",
		reasons: ["反馈包含明确的修改对象、位置、参数或替换动作。"]
	};
	return {
		classification: "ambiguous_creative",
		reasons: ["反馈没有形成可确定执行的局部编辑指令。"]
	};
}
...
function validateRevisionInput(payload) {
	...
	if (typeof input.current_prompt !== "string" || !input.current_prompt.trim()) throw new Error("current_prompt must be a non-empty string");
	if (typeof input.user_feedback !== "string" || !input.user_feedback.trim()) throw new Error("user_feedback must be a non-empty string");
	const locked = input.locked_context;
	if (!locked || typeof locked !== "object") throw new Error("locked_context must be an object");
	if (!Array.isArray(locked.contract_rules) || !locked.contract_rules.every((item) => typeof item === "string" && item.trim())) throw new Error("locked_context.contract_rules must be a list of non-empty strings");
	if (!Array.isArray(locked.material_order) || !locked.material_order.every((item) => typeof item === "string" && item.trim())) throw new Error("locked_context.material_order must be a list of non-empty strings");
	if (!VALID_REVISION_RATIOS.includes(locked.ratio)) throw new Error(`locked_context.ratio must be one of: ${[...VALID_REVISION_RATIOS].join(", ")}`);
	const duration = locked.duration_seconds;
	if (typeof duration !== "number" || !Number.isFinite(duration) || duration <= 0) throw new Error("locked_context.duration_seconds must be a positive number");
}
/** Build a constrained DT revision request (classify only; no rewrite/corpus/media). */
function buildRevisionRequest(payload) {
	...
	const scope = classification === "explicit_local" ? "仅修改用户明确指出的局部内容" : "按用户反馈优化，但保留所有锁定上下文";
	return {
		schema_version: "1.0",
...
function validateRevisionResult(result, request) {
	const errors = [];
	const r = result;
	...
	if (![
		...
		"ambiguous_creative",
		"structural_rewrite"
	].includes(r.classification)) errors.push("classification must be explicit_local | ambiguous_creative | structural_rewrite");
	if (typeof r.revised_prompt !== "string" || !r.revised_prompt.trim()) errors.push("revised_prompt must be a non-empty string");
	if (!Array.isArray(r.changed_sections) || !r.changed_sections.every((s) => typeof s === "string" && s.length > 0)) errors.push("changed_sections must be a list of non-empty strings");
	if (r.preserved_unspecified_content !== true) errors.push("preserved_unspecified_content must be true");
	if (typeof r.locked_context_sha256 !== "string" || !/^[0-9a-f]{64}$/.test(r.locked_context_sha256)) errors.push("locked_context_sha256 must be a 64-char hex string");
	if (request && r.locked_context_sha256 !== request.locked_context_sha256) errors.push("locked_context_sha256 does not match the request (stale contract; reject)");
	const usage = r.corpus_usage;
	if (!usage || typeof usage !== "object") errors.push("corpus_usage is required");
	else {
		...
		for (const match of matches) {
			...
			if (typeof match.portable_pattern !== "string" || !match.portable_pattern) errors.push("every corpus match requires portable_pattern");
		}
		if (r.classification === "explicit_local" && matches.length > 0) errors.push("explicit_local must not use corpus matches");
	}
	return {
		ok: errors.length === 0,
		errors
...
//#endregion
export { validateRevisionInput as n, validateRevisionResult as r, buildRevisionRequest as t };
