...
> 本流程的提示词创作**必须先检索语料、通过 `prompt_revision authoring_gate` 创作门**，不得凭印象直接写（对应 Bug 总汇 BUG-02）。写作门槛与 `dt-video-prompt` 编排器一致。
