...
4. **提示词 V1**：由业务 Skill 产出中文提示词 V1，用 `set_prompt`（source=skill_v1）写入，然后 `confirm_prompt` 锁定素材哈希与提示词哈希。
5. **修订**：用户要求修改时 `request_revision`（explicit_local 明确局部修改跳过语料；ambiguous_creative / structural_rewrite 最多参考 3 条相关语料）→ `begin_revision` → 修订后 `set_prompt`（source=dt_revision）→ 再次 `confirm_prompt`。**每个版本都必须再次确认。**
