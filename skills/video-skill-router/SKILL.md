...
description: 视频业务 Skill 线（Codex_CS 流程）。**仅当用户显式要求启用 Skill 模式时使用**（如"启用 skill 模式""用业务 Skill""走 skill 线""用 XX 业务包"）；默认视频创作一律走导演线 video-prompt-orchestrator，本技能不得被主动触发。进入后第一句必须明确告知用户"已进入 Skill 模式"。流程：检索/确认 Skill → 确认比例时长 → 建项目（project_pipeline create 的 skill_mode 硬门）→ 收集素材 → 提示词 V1 → 确认 → 提交视频。业务 Skill 由 skill_registry 治理。
whenToUse: 用户在当前请求中明确要求启用 Skill 模式或使用业务 Skill 创作视频时。用户没有这样要求时不要使用本技能。
---

# 视频业务 Skill 线（Codex_CS 流程，显式启用）

> **触发门（动手前先判定）**：只有用户在当前请求中**显式要求启用 Skill 模式**（"启用 skill 模式""用业务 Skill""走 skill 线""用 XX 业务包"）才使用本技能。
> - 用户**没有**这样要求时：**不得**检索业务 Skill、**不得**调用 `skill_registry` / `project_pipeline`，改走导演线 `video-prompt-orchestrator`。
> - 用户这样要求时：**第一句先明确告知用户"已进入 Skill 模式"**，再开始检索。
> - `project_pipeline create` 有 `skill_mode` 硬门：不传 `skill_mode=true` 会被直接拒绝，这是代码级强制，不是文档约定。

1. **告知 + 检索**：先输出"已进入 Skill 模式"，再用 `skill_registry` 的 `search` 按用户创作意图检索已发布业务 Skill；把候选列给用户。**素材不能作为主要路由依据**——先定 Skill，再谈素材。
2. **确认 Skill、比例、时长**：用户明确确认 Skill 名后，用 `project_pipeline` 走状态机：`create`（**必须带 `skill_mode=true`**）→ `confirm_skill` → `set_settings`（比例与时长 4-30 秒）。各 Skill 素材槽数量由各自 contract 的 `count_rule` 推导，**不用统一"每秒几张图"规则**。
...
5. **修订**：用户要求修改时自动进入受约束修订：`project_pipeline` 的 `request_revision` 传 `feedback` 原文 → 确定性分类并存储受约束修订请求（explicit_local 明确局部修改跳过语料；ambiguous_creative / structural_rewrite 最多检索 10 条相关语料，用 `prompt_revision` 的 `search_corpus`）→ `begin_revision` → 修订后 `set_prompt`（source=governed_revision）→ 再次 `confirm_prompt`。**每个版本都必须再次确认；修订结果必须回显同一 locked_context_sha256（`prompt_revision` 的 `validate_result` 校验）。**
