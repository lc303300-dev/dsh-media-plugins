...
| `split_grid_sheet` | 3×3 九宫格拆格（**双检测器 + 仲裁**）：方案1 形态学线检测（阈值+整幅白色行长带，纯白格线最快最准）→ 方案2 **亮度曲线峰值检测**（整行/整列平均亮度取「最接近 1/3、2/3 的**窄峰**」——以**峰宽**而非绝对亮度拒绝把大片亮内容（天空/水面/白墙）误判为格线，可处理浅灰/柔和格线、行高列宽不均匀）→ 两者结果按「谁更接近标准三等分」仲裁，皆失败或**偏离 >20% 判定不可靠**时回退方案3 等比分割。检测成功时按格线带**外侧**切割，格线像素零残留（此前的「格线中心 ± 内缩」在行高不均时会切进相邻格）。**支持批量分组**：`groups=[{group, images}]` 把每组所有图的面板**平铺**写入 `<output_dir>/<group>/`，组内不再嵌套子目录。可选 normalize_ratio 规范比例；单张输出 r1c1..r3c3 + 自包含审阅页 | sharp | 无 |
