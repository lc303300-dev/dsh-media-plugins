...
function apply(ctx, config) {
	ctx.tools.register(defineTool({
		name: "dt_batch",
		description: "DT 批次工作台（Codex_DT 的 DSH 重建）：创建隔离批次、生成 1024px 预览、初始化 manifest（时长/比例/模型选择证据/用户运镜与镜头要求/素材路径）、逐素材写入可执行中文提示词并生成 review/index.html 供用户逐项确认。最终脚本只调用统一媒体工具，不直接调用供应商。",
		parameters: {
			command: {
				type: "string",
				enum: [
			...
			batch_id: {
				type: "string",
				description: "批次 id（init 缺省自动生成）。"
			},
			duration: {
				type: "integer",
				description: "init_batch 用：视频时长 4-30 秒。"
			},
			ratio: {
				type: "string",
				description: "init_batch 用：视频比例。"
			},
			model: {
				type: "string",
				description: "init_batch 用：模型选择证据（如 seedance2.5）。"
			},
			user_requirements: {
				type: "string",
				description: "init_batch 用：用户运镜/镜头/时长要求。"
			},
			materials: {
				type: "array",
				items: { type: "string" },
				description: "init_batch 用：本地素材路径列表（顺序即素材编号）。"
			},
			prompts: {
				type: "array",
				items: {
