import { a as readYamlFile, o as validateCodexFlowPackage } from "./flow-format.js";
import { basename, isAbsolute, join, relative, resolve } from "node:path";
import { createHash } from "node:crypto";
import { copyFileSync, existsSync, mkdirSync, readFileSync, readdirSync, statSync, writeFileSync } from "node:fs";
//#region src/shared/image-skill-core.ts
/**
...
*/
const IMAGE_VALIDATOR_VERSION = "2.0.0";
const IMAGE_HASH_ALGORITHM = "codex-is-package-sha256-v2";
const IMAGE_TEXT_EXTENSIONS = /* @__PURE__ */ new Set([
	".md",
	".json",
	".yaml",
	".yml",
...
const IMAGE_SKILL_ID_PATTERN = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
const IMAGE_SHA256_PATTERN = /^[a-f0-9]{64}$/;
const IMAGE_TEMPLATE_MARKERS = /(\{\{[^}]+\}\}|CURATOR-REQUIRED|\bTODO\b|\bTBD\b|START OF FILE)/i;
const IMAGE_FORBIDDEN = /(API[_ -]?KEY|authorization\s*header|cookie\s*=|dreamina\s*cli|provider adapter|polling|download loop)/i;
/** SHA-256 of a file's raw bytes. */
function imageFileSha256(path) {
...
function imageCanonicalBytes(path) {
	const data = readFileSync(path);
...
function imagePackageSha256(root, exclude = /* @__PURE__ */ new Set(["intake-receipt.json"])) {
...
function imageCoreSha256(root) {
	return imagePackageSha256(root, /* @__PURE__ */ new Set(["intake-receipt.json", "intake-report.json"]));
}
/** Parse the YAML frontmatter of a SKILL.md; only name/description matter. */
function imageParseFrontmatter(text) {
...
const REFERENCE_POLICY_REQUIRED = [
	"allowed_slot_ids",
	"reject_uncontracted_images",
	"maximum_reference_images_per_scene",
	"preserve_reference_order"
];
const WORKLOAD_REQUIRED = [
	"scene_count",
	"candidate_count_per_scene",
	"batch_allowed"
];
const COUNT_RANGE_REQUIRED = ["min", "max"];
const OUTPUT_REQUIRED = [
	"media_type",
	"requires_ratio_confirmation",
	"supported_ratios"
];
const AUTHORING_REQUIRED = [
	"primary_language",
	"requires_reference_binding",
	"requires_prompt_confirmation",
	"user_instruction_priority"
];
const EXECUTION_REQUIRED = [
	"provider_neutral",
	"single_candidate_entry",
	"batch_entry",
	"requires_paid_batch_confirmation",
	"automatic_retry",
	"automatic_visual_ranking"
];
...
function exactKeys(instance, required, label, issues, prefix = "") {
...
function validateImageContractShape(contract) {
	const issues = [];
	if (!isPlainObject(contract)) {
		issues.push("CONTRACT_SCHEMA: contract must be an object");
		return issues;
	}
	const label = (path) => `CONTRACT_SCHEMA:${path}:`;
	...
	contract.references.forEach((item, index) => {
		...
		for (const field of [
			...
			"observation_required",
			"send_to_generation"
		]) if (typeof item[field] !== "boolean") issues.push(`${label(path + "." + field)}must be boolean`);
...
function validateImageRoutingShape(routing) {
	const issues = [];
...
function validateImageReportShape(report) {
	const issues = [];
	...
	if (!isPlainObject(approval) || !exactKeys(approval, [
		"required",
		"approved",
		"approved_by",
		"approved_at"
	], "user_approval", issues, "REPORT_SCHEMA:")) return issues;
...
function validateImageReceiptShape(receipt) {
	const issues = [];
	...
	if (receipt.hash_algorithm !== "codex-is-package-sha256-v2") issues.push(`RECEIPT_SCHEMA:hash_algorithm:must be ${IMAGE_HASH_ALGORITHM}`);
...
function validateImagePackage(root, options = {}) {
	const issues = [];
	const required = [
		"SKILL.md",
		"contract.json",
		"routing.json",
		"references/creative-guidance.md",
	...
	for (const relative of required) if (!existsSync(join(root, relative))) issues.push(`MISSING:${relative}`);
	...
	for (const key of ["scene_count", "candidate_count_per_scene"]) {
		const bounds = isPlainObject(workload[key]) ? workload[key] : {};
	...
	const packageText = listFiles(root).filter((path) => IMAGE_TEXT_EXTENSIONS.has(path.slice(path.lastIndexOf(".")).toLowerCase())).map((path) => readFileSync(path, "utf8")).join("\n");
...
function validateImageReceipt(root, expectedSkillId) {
	...
	try {
		...
		return {
			receipt: null,
			issues: ["INVALID_RECEIPT"]
		};
	}
	const issues = [];
	if (receipt.schema_version !== 2 || receipt.hash_algorithm !== "codex-is-package-sha256-v2") issues.push("UNSUPPORTED_RECEIPT_SCHEMA");
...
function isCodexFlowImagePackage(root) {
...
function validateCodexFlowImagePackage(root, published = false) {
	...
	if (!(Array.isArray(meta.capabilities) ? meta.capabilities.map(String) : []).includes("image.generate")) issues.push("FLOW_IMAGE_MISSING_CAPABILITY");
	...
	if (existsSync(workflowPath)) {
		...
		for (const stage of stages) {
			...
			if (typeof record.capability === "string" && record.capability.trim().length > 0) {
				...
				if (gate !== "paid-execution" && gate !== "batch-approval") issues.push(`FLOW_IMAGE_INVALID_PRODUCTION_GATE:${String(record.id ?? "?")}:${gate}`);
			}
		}
	}
...
function flowSkillToImageContractShape(flow, name, displayName, description) {
	const batchAllowed = (Array.isArray(flow.capabilities) ? flow.capabilities.map(String) : []).includes("image.batch-generate");
...
function approveImageIntakeReport(packageDir) {
	...
	if (report.status !== "ready_for_approval" || Array.isArray(report.blocking_questions) && report.blocking_questions.length > 0 || Array.isArray(report.contract_conflicts) && report.contract_conflicts.length > 0 || Array.isArray(report.validation_issues) && report.validation_issues.length > 0) throw new Error("Report is not ready for approval");
...
function buildImageIntakeReceipt(skillId, sources, stagingDir, reviewedCoreSha256) {
	return {
		schema_version: 2,
...
function stageImagePublish(packageDir, stagingRoot, sources) {
	...
	if (report.status !== "approved" || !isPlainObject(report.user_approval) || report.user_approval.approved !== true || report.reviewed_core_sha256 !== imageCoreSha256(packageDir)) throw new Error("Package is not bound to a current user approval");
	...
	const stagedReport = JSON.parse(readFileSync(stagedReportPath, "utf8"));
	...
	const post = validateImagePackage(stagingDir, {
		requireReport: true,
		requireReceipt: true
	});
...
function scaffoldImageSkill(templateRoot, skillId, outputRoot, displayName, description) {
...
function imageContractToRegistryContract(contract, routing, version) {
	const references = Array.isArray(contract.references) ? contract.references : [];
	const workload = isPlainObject(contract.workload) ? contract.workload : {};
	...
	return {
		...
		image: {
			...
			scene_count: isPlainObject(workload.scene_count) ? workload.scene_count : {
				min: 1,
				max: 1
			},
			candidate_count_per_scene: isPlainObject(workload.candidate_count_per_scene) ? workload.candidate_count_per_scene : {
				min: 1,
				max: 1
			},
...
//#endregion
export { imageContractToRegistryContract as a, isCodexFlowImagePackage as c, stageImagePublish as d, validateCodexFlowImagePackage as f, flowSkillToImageContractShape as i, scaffoldImageSkill as l, validateImageReceipt as m, approveImageIntakeReport as n, imageFileSha256 as o, validateImagePackage as p, auditImageSkill as r, imagePackageSha256 as s, IMAGE_SKILL_ID_PATTERN as t, sealImageSources as u };
