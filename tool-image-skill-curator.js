...
//#region src/tool-image-skill-curator.ts
...
function libraryRoot(workspaceRoot, privateRoot, configured) {
...
async function copyTree(src, dst) {
	...
	for (const entry of entries) {
		...
		if ((await stat(from)).isDirectory()) await copyTree(from, to);
		else await copyFile(from, to);
	}
}
/** Copy a package into the private library root (seed_library). */
async function copyPackageToLibrary(source, libraryDir) {
		...
		async execute(args, exec) {
			...
			try {
				if (command === "seed_library") {
					...
					if (!existsSync(pluginLib)) return {
						ok: false,
						message: `plugin image-skill-library not found: ${pluginLib}`
					};
					const seeded = [];
					const failed = [];
					for (const entry of await readdir(pluginLib)) {
						...
						try {
							...
							const issues = validateImagePackage(copied, {
								requireReport: true,
								requireReceipt: true
							});
							if (issues.length > 0) {
				...
				if (command === "scaffold") {
					...
					if (!IMAGE_SKILL_ID_PATTERN.test(skillId) || skillId.length > 63) return {
						ok: false,
				...
				if (command === "audit") {
					...
					const sources = Array.isArray(args.sources) && args.sources.length > 0 ? args.sources.map((p) => resolvePath(String(p))) : [];
					...
					return {
						ok,
						message: ok ? "intake-report written (ready_for_approval)" : `intake-report written (needs_review, ${report.validation_issues.length} issue(s))`,
						report,
						package_path: packageDir,
				...
				if (command === "approve") {
					...
					try {
						...
						return {
							ok: true,
							message: "intake-report approved by user",
							report,
							package_path: packageDir
						};
					} catch (error) {
				...
				if (command === "publish" || command === "upgrade") {
					...
					if (args.approved !== true) {
						...
						return {
							...
							checklist: {
								...
								source_hashes: sealedSources.map((s) => ({
									name: s.name,
									sha256: s.sha256.slice(0, 16) + "…"
								})),
								slots: (Array.isArray(report.reference_summary) ? report.reference_summary : []).map((item) => ({
									id: item.id,
									role: item.role,
									scope: item.scope,
					...
					if (Array.isArray(args.sources) && args.sources.length > 0) sources = args.sources.map((p) => ({
						name: basename(String(p)),
						sha256: imageFileSha256(resolvePath(String(p)))
					}));
					if (sources.length === 0) return {
					...
					try {
						...
						if (moved) await rm(join(libRoot, skillId), {
							recursive: true,
							force: true
						}).catch(() => void 0);
						return {
