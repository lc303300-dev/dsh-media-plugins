/**
 * Preview tool: EXIF-normalized, ≤1024 px preview of a local image, plus
 * a dimensions report. Visual checks and review pages use this instead of
 * inspecting originals (Codex_image "1024 px preview" guardrail).
 *
...
import { makePreview, imageDimensions } from './shared/image-ops.ts'

/** Cordis plugin name used by loader diagnostics. */
export const name = 'Ws_tool-preview'
...
function apply(ctx: Context, config: ResolvedConfig): void {
  ctx.tools.register(
    defineTool({
      name: 'image_preview',
      description:
        '生成本地图片的 EXIF 方向归一化预览（最长边 ≤ 1024px，默认）并返回原图与预览的尺寸信息。视觉检查、审阅页与 describe_image 均应基于预览而非原始大图；原图永不被覆盖或修改。',
      parameters: {
        image: {
          type: 'string',
          required: true,
        ...
        max_long_edge: { type: 'integer', description: '可选：预览最长边上限，默认 1024。' },
      ...
      output: {
        ...
        render(_args: unknown, value: any) {
          return [{ type: 'text', text: value.message ?? `preview: ${value.preview_path}` }]
        },
      },
      async execute(args: any, exec: any) {
        ...
        try {
          ...
          if (args.output && String(args.output).trim().length > 0) {
            ...
            preview = { ...preview, path: finalPath }
          }
          return { ok: true, original, preview_path: preview.path, preview, message: `preview ready: ${preview.path} (${preview.width}x${preview.height})` }
        } catch (error: any) {
