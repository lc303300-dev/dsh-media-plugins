  ...
  ctx.tools.register(
    defineTool({
      ...
      parameters: {
        command: {
          type: 'string',
          enum: ['scaffold', 'validate', 'add_count_rules', 'planned_counts', 'migrate', 'discover', 'publish', 'prepare_creative_supplement', 'receive_creative_supplement', 'approve_creative_supplement'],
          required: true,
          description: '操作命令。',
        },
        ...
        draft_path: { type: 'string', description: 'receive_creative_supplement 用：创意补充草稿 JSON 路径。' },
        approved_by: { type: 'string', description: 'approve_creative_supplement 用：必须为 user。' },
      ...
      async execute(args: any, exec: any) {
        ...
        if (command === 'prepare_creative_supplement') {
          ...
          writeFileSync(join(reviewDir, 'creative-supplement-request.json'), JSON.stringify(request, null, 2) + '\n', 'utf8')
        ...
        if (command === 'receive_creative_supplement') {
          ...
          if (draft.draft !== true) return { ok: false, message: '创意补充必须是草稿（draft=true），未经批准不得进入正式 references' }
          const reviewDir = join(packageDir, 'review')
          mkdirSync(reviewDir, { recursive: true })
          writeFileSync(join(reviewDir, 'creative-supplement-draft.json'), JSON.stringify(draft, null, 2) + '\n', 'utf8')
        ...
        if (command === 'approve_creative_supplement') {
          if (args.approved_by !== 'user') return { ok: false, message: 'approve 必须由用户明确批准（approved_by=user）' }
          const reviewDir = join(packageDir, 'review')
          let draft: any
          try {
            draft = JSON.parse(readFileSync(join(reviewDir, 'creative-supplement-draft.json'), 'utf8'))
          } catch {
            return { ok: false, message: 'no received draft; run receive_creative_supplement first' }
          }
