 ...
 */
async function isFlowSkill(privateRoot: string, skillName: string): Promise<boolean> {
...
async function applySlotPlans(
  ...
  try {
    ...
    // Codex_Flow 包（无 contract.json）：用 flow 元数据合成通用契约，槽 = 合成 slots
    // （flow 包不声明素材槽/count_rule，SlotPlan 按通用默认推导，保持现有 planSlots 逻辑）。
    let refs: Array<Record<string, any>>
    if (isFlowVideoSkill(rec.contract)) {
      ...
      refs = (synthesized.slots ?? []) as Array<Record<string, any>>
    } else {
...
export function slotDirOutput(plans: SlotPlan[] | undefined): Array<Record<string, unknown>> {
      ...
      async execute(args: any, exec: any) {
        ...
        if (command === 'create') {
          ...
          if (args.skill_name) {
            ...
            state = { ...state, skillFormat: flow ? 'flow' : 'legacy' }
          }
          ...
          await save(state)
          return { ok: true, message: `project ${id} created (${state.status})`, project: state, slot_dirs: slotDirOutput(state.slotPlans) }
        }
        ...
        switch (command) {
          case 'confirm_skill': {
            ...
            // 创建边界识别 Codex_Flow 视频包（contract.flow 存在且 capabilities 含 video.generate）
            const flow = await isFlowSkill(privateRoot, String(args.skill_name))
            const next = {
          ...
          case 'set_settings': {
            ...
            return { ok: true, message: `status -> ${next.status}（${next.slotPlans?.length ?? 0} 个素材槽已规划）`, project: await save(next), slot_dirs: slotDirOutput(next.slotPlans) }
          }
          ...
          case 'add_material': {
            ...
            // （flow 包无 contract.json，用 flow 元数据合成通用契约的槽）
            ...
            if (state.skillName) {
              ...
              try {
                const rec = registry.get(state.skillName)
                if (rec) {
                  slots = isFlowVideoSkill(rec.contract)
                    ? (synthesizeVideoContractFromFlow(rec.contract.flow!, {
                        name: rec.name,
                        version: rec.version,
                        description: rec.description,
                        taxonomy: rec.taxonomy,
                      }).slots as SlotContract[] | undefined)
                    : rec.contract.slots
                }
              } finally {
                registry.close()
