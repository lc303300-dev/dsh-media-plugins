 ...
 * gifsicle, denoise/anti-moire/color tuning, duration cap, directory batch
 * and a CSV conversion report. Pure local processing — never a paid call.
 *
...
import { videoToGif, resolveFfmpeg, type GifMode, type DitherMode, type DenoiseLevel } from './shared/gif-core.ts'
import { resolvePrivateRoot, ensureDir } from './shared/private-runtime.ts'
...
async function listVideos(dir: string, recursive: boolean, depth = 0): Promise<string[]> {
  const out: string[] = []
  let entries
  try {
  ...
  ctx.tools.register(
    defineTool({
      ...
      parameters: {
        ...
        recursive: { type: 'boolean', description: '批量时是否递归子目录。' },
        ...
        min_width: { type: 'integer', description: 'quality 模式的最低尝试宽度（默认 360）。' },
        dither: {
          type: 'string',
          enum: ['bayer', 'sierra2_4a', 'floyd_steinberg', 'none'],
          description: '抖动模式，默认 bayer。',
        },
        bayer_scale: { type: 'integer', description: 'bayer 抖动的 bayer_scale（0-5，默认 5）。' },
        palette_stats_mode: { type: 'string', enum: ['diff', 'full', 'single'], description: 'palettegen stats_mode，默认 diff。' },
        diff_mode: { type: 'string', enum: ['rectangle', 'none'], description: 'paletteuse diff_mode，默认 rectangle。' },
        color_count: { type: 'integer', description: '调色板最大颜色数（max_colors）。' },
        max_duration_sec: { type: 'number', description: '可选：截断输入到最多 N 秒（长视频）。' },
        denoise: { type: 'string', enum: ['off', 'light', 'medium'], description: 'hqdn3d 降噪：off/light/medium，默认 off。' },
        anti_moire: { type: 'boolean', description: '缩放使用 accurate_rnd 抗摩尔纹。' },
        lossy: { type: 'integer', description: 'gifsicle --lossy 0-200（需 gifsicle_path，-O3 --careful）。' },
        gifsicle_path: { type: 'string', description: '可选：gifsicle 二进制路径（用于 lossy 优化）。' },
        output: { type: 'string', description: '可选输出路径（绝对或相对会话工作目录；单文件时有效）。' },
      },
      output: {
        schema: {
          type: 'object',
      ...
      async execute(args: any, exec: any) {
        ...
        if (!video && !inputDir) return { ok: false, message: 'video path or input_dir is required' }

        const gifsiclePath = String(args.gifsicle_path ?? config.gifsiclePath).trim() || undefined
        const baseOptions = {
          width: args.width,
          fps: args.fps,
          maxSizeMB: args.max_size_mb ?? config.maxSizeMB,
          mode: (args.mode ?? 'quality') as GifMode,
          minWidth: args.min_width,
          dither: (args.dither ?? 'bayer') as DitherMode,
          ...
          denoise: (args.denoise ?? 'off') as DenoiseLevel,
        ...
        const targets = video
          ? [{ video: video.trim(), isSingle: true }]
          : (await listVideos(isAbsolute(inputDir) ? inputDir : join(workspaceRoot, inputDir), Boolean(args.recursive)))
              .map((v) => ({ video: v, isSingle: false }))
        if (targets.length === 0) return { ok: false, message: inputDir ? 'no video files found in input_dir' : 'video file not found' }

        const outDir = join(workspaceRoot, config.outputDir, 'gif')
        await mkdir(outDir, { recursive: true })
        const reportDir = join(privateRoot, 'reports')
        await ensureDir(reportDir)
        const rows: string[][] = [['input', 'status', 'reason', 'width', 'fps', 'size_bytes', 'within_budget']]
        const results: any[] = []
        ...
        for (const target of targets) {
          try {
            ...
            rows.push([target.video, 'success', '', String(result.width), String(result.fps), String(result.sizeBytes), String(result.withinBudget)])
            results.push({
              input: target.video,
              ok: true,
              path: finalPath,
            ...
            rows.push([target.video, 'failed', reason, '', '', '', ''])
            results.push({ input: target.video, ok: false, message: reason })
          }
        }
        ...
        if (targets.length === 1 && targets[0].isSingle) {
          ...
          if (!single.ok) return { ok: false, message: single.message, report_path: reportPath }
          return { ...single, report_path: reportPath, message: `gif ready: ${single.path} (${Math.round(single.size_bytes / 1024)}KB, ${single.width}px@${single.fps}fps, ${single.attempts} attempt(s))` }
        }
