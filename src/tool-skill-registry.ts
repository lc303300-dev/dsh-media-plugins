...
function apply(ctx: Context, config: ResolvedConfig): void {
  ctx.tools.register(
    defineTool({
      name: 'skill_registry',
      description:
        '业务 Skill 治理（Codex_CS 的 DSH 重建）：摄取/检索/获取/发布/弃用业务视频 Skill。ingest 从 package_dir 读取 SKILL.md（frontmatter）、contract.json（名称、版本、视频比例/时长、素材槽 min/max/planned_count/count_rule）与 routing.json，做结构校验与去重（name@version + 内容哈希），发布前为 draft。search 用 SQLite FTS5 trigram 做中文友好检索（默认只搜已发布）。检索以用户创作意图为主，素材不作为主要路由依据。',
      parameters: {
        command: {
          type: 'string',
          enum: ['ingest', 'search', 'get', 'publish', 'deprecate', 'list'],
          required: true,
